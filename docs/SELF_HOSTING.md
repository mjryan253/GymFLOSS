# Self-hosting GymFLOSS

GymFLOSS is two small containers (a web server and an API) plus a folder of your data.
...
```bash
git clone https://github.com/mjryan253/GymFLOSS gymfloss
cd gymfloss
cp .env.example .env
...
GymFLOSS signs you in with **passkeys** (WebAuthn). Browsers enforce two rules:
...
passkey prompt won't appear. To use GymFLOSS from your phone you need a real HTTPS hostname.
...
Put GymFLOSS behind something that terminates TLS for a hostname you control, then point it at
...
GymFLOSS only needs the browser to reach it over `https://gym.example.com`.
...
```bash
tar czf gymfloss-backup-$(date +%F).tar.gz data/
```
...
GymFLOSS can push two kinds of alert to your phone/desktop, even when the app isn't open:
